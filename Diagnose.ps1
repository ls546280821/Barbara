# ============================================================================
#  Diagnose.ps1  --  Barbara environment check
#
...
#
#  ASCII-ONLY ON PURPOSE -- see the long explanation in Start-Barbara.ps1.
...
# The launcher was renamed from Start-Cyrene.ps1 to Start-Barbara.ps1;
# check whichever one is present so the report stays useful either way.
foreach ($name in 'Start-Barbara.ps1', 'Start-Cyrene.ps1') {
...
# ---------- Barbara data folder ----------
Add-Line '--- Barbara data folder ---'
$dataDir = Join-Path $env:APPDATA 'Barbara'
if (Test-Path -LiteralPath $dataDir) {
  ...
  Add-Line '(Barbara has never started successfully on this machine)'
}
Add-Line ''

# ---------- legacy Cyrene data folder ----------
Add-Line '--- legacy Cyrene data folder (pre-rename) ---'
$legacyDir = Join-Path $env:APPDATA 'Cyrene'
if (Test-Path -LiteralPath $legacyDir) {
  Add-Line ('Exists: ' + $legacyDir)
  Get-ChildItem -LiteralPath $legacyDir -File -ErrorAction SilentlyContinue |
    ForEach-Object { Add-Line ('  ' + $_.Name + '  (' + $_.Length + ' bytes)') }
  Add-Line '(This data is copied to the Barbara folder on first launch.)'
} else {
