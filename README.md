...
>
> `renderer/js/main.js` 仍然不小（**7100+ 行**，大部分界面逻辑还在里面）。
> 它已切成 ES module，抽出了 `core/` `ui/` `data/` 三层，**刷新总线也已就位**
> （`views/refresh.js`）—— 下一步是按功能把 `views/` 分出来。
