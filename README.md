...
│       └── views/       一个功能一块（refresh 总线 / header / perspectiveUi / player / memoryUi）
...
>
> `renderer/js/main.js` 仍然不小（**6400+ 行**，大部分界面逻辑还在里面）。
> 它已切成 ES module，抽出了 `core/` `ui/` `data/` 三层，刷新总线已就位
> （`views/refresh.js`），**views 已拆掉 4 个功能模块** ——
> `header`（对话头部）/ `perspectiveUi`（视角设置）/ `player`（玩家角色弹窗）/
> `memoryUi`（记忆 + 存档点）；接下来是 `panelUi` / `worldbook` / `chat` 等。
