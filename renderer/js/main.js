...
const PACE_MODES = {
  step: {
    ...
    text:
      ...
      '\n' +
      '一轮里最多包含：\n' +
      '  1. 描写玩家这句输入直接造成的眼下变化（1 段，两三句）\n' +
      '  2. 在场 NPC 各自的一句话或一个动作（每人最多一次，不要替他们连续互动）\n' +
      '然后立刻停住，等玩家回应。\n' +
      '\n' +
      '★ 篇幅也按这个来：一轮就是一到两段。宁可少写，让玩家接着往下推。\n' +
      '  不要在一轮里堆细节（外貌、环境、酒馆里的其他人），那会把节奏一起拖快。\n' +
      '\n' +
      '★★ 最重要的一条：在对峙里不要替玩家把局面解决掉。\n' +
      '  你可以描写局势和 NPC 的反应，但「这事怎么了结」必须留给玩家。\n' +
      '  凡是还在进行中的对峙、交涉、争论 —— 一轮只写到「对方有了反应」，\n' +
      '  不要让它当场分出结果，也不要让任何一方单方面结束这段互动。\n' +
      '  具体来说，不要做这些：\n' +
      '   · 让对手 / 关键 NPC 突然离场、认输、摊牌、把话说完就走\n' +
      '   · 让别的 NPC 走过来接过话头，把玩家原本在进行的互动接管掉\n' +
      '   · 替玩家化解尴尬、结束冲突、或者替他把事情推向某个结论\n' +
      '  玩家的输入表示「他打算做什么」，不是把后续都授权给你。\n' +
      '  不要替玩家决定具体做法、不要替玩家说话、不要替他做选择。\n' +
      '\n' +
      '★★ 但这条只管「剧情怎么发展」，不管「已经说定的事」：\n' +
      '  玩家已经明确要了什么、已经点头同意的事，NPC 就该直接做，\n' +
      '  一轮里把它推进到位，不要停下来反复确认。\n' +
      '  明确禁止这些拖延：\n' +
      '   · 把已经答应的事再问一遍「可以吗」「你确定吗」\n' +
      '   · 在玩家没提出选择的地方硬造一个选择丢回给他\n' +
      '   · 把「谁先来」「按什么顺序」这类属于 NPC 自己的安排，抛给玩家决定\n' +
      '  NPC 之间的先后、分工、争执是 NPC 的事，由你安排到位；\n' +
      '  玩家的选择只用在「玩家自己要做什么」上。\n' +
      '\n' +
      '★ 判断标准：玩家这句话是在「挑起 / 试探 / 还没表态」，\n' +
      '  还是在「明确要求某个动作」？\n' +
      '  前者 —— 停下，把反应写出来等他回应。\n' +
      '  后者 —— 直接做到位，别再回头问。\n' +
      '\n' +
      '★ 凡是「过程型」的情节 —— 一局游戏、一场谈判、一次比试、一顿饭、一段赶路 ——\n' +
      '  一轮只写到「开场」为止：约好了玩什么、摆好架势、对方应了一声，就停下。\n' +
      '  后面的每一手、每一局、每一个回合，都留给玩家逐个决定。\n' +
      '  绝对不要在一轮里把整局玩完、也不要替玩家掷骰子或出牌。\n' +
      '\n' +
      '★ 不要主动引入玩家没提到的新角色或新事件。第三方插手、剧情转折这类事，\n' +
      '  要等玩家先给了机会再发生。已经出场的 NPC 也不要自己凑过来改变局面 ——\n' +
      '  除非玩家先跟他们说了话、或者剧情已经把他们卷了进来。\n' +
      '\n' +
      '每轮结尾停在需要玩家表态的地方 —— 但前提是这一轮里确实\n' +
      '还有「需要玩家拍板的事」。玩家已经拍过板的，就别再问第二遍。'
  },
  brisk: {
    label: '快节奏',
    hint: '允许一轮里多推进一些',
    text:
      ...
      '结尾仍然要停在一个需要玩家回应的位置（除非这一轮里已经没有需要他拍板的事）。'
  }
};
